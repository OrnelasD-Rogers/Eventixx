...
class EventIndexServiceTest {
  ...
  @Mock private EventDocumentRepository repository;

  @Mock private EventDocumentMapper mapper;

  @InjectMocks private EventIndexService eventIndexService;

  @Captor private ArgumentCaptor<EventDocument> documentCaptor;

  @Test
  void shouldIndexEvent() {
    var event = createSampleEvent();
    var doc =
        EventDocument.builder().eventId(event.eventId().toString()).title(event.title()).build();
