...
class SearchControllerWebTest {
  ...
  void shouldReturnEventsWhenQueryProvided() throws Exception {
    ...
    var result =
        new SearchResult(
            List.of(event),
            new Facets(Map.of("Music", 1L), Map.of("Sao Paulo", 1L)),
            "cursor123",
            1L);
    ...
    mockMvc
        .perform(get("/api/v1/search/events").param("q", "rock").header("X-User-Id", "user-123"))
  ...
  void shouldReturnEmptyResultWhenNoMatches() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/search/events").param("q", "nonexistent").header("X-User-Id", "user-123"))
        .andExpect(status().isOk())
  ...
  void shouldReturn503WhenSearchServiceUnavailable() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/search/events").param("q", "test").header("X-User-Id", "user-123"))
  ...
  void shouldReturnEventsWithFiltersAndReturnResult() throws Exception {
    ...
    var result =
        new SearchResult(
            List.of(event),
            new Facets(Map.of("Music", 1L), Map.of("Rio de Janeiro", 1L)),
            null,
            1L);
