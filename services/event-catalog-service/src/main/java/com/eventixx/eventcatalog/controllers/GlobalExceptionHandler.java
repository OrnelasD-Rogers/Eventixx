...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail handleValidationException(MethodArgumentNotValidException ex) {
        ...
        ProblemDetail problem = ProblemDetail.forStatusAndDetail(
            ...
            "Request body contains " + errorCount + " validation error(s). Check 'errors' for details."
        );
        problem.setType(ProblemType.VALIDATION_ERROR);
        problem.setTitle(ProblemType.TITLE_VALIDATION_ERROR);
        List<ValidationErrorDetail> errors = new ArrayList<>();
        for (FieldError fieldError : ex.getBindingResult().getFieldErrors()) {
            errors.add(new ValidationErrorDetail(fieldError.getDefaultMessage(), "/" + fieldError.getField()));
        }
