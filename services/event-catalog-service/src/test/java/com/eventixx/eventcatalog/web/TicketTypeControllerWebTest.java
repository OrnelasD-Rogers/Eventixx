...
class TicketTypeControllerWebTest {
  ...
  void shouldCreateTicketType() throws Exception {
    CreateTicketTypeRequest request =
        new CreateTicketTypeRequest("VIP", BigDecimal.valueOf(200), 50);
    TicketTypeResponse response =
        new TicketTypeResponse(
            ticketTypeId, "VIP", BigDecimal.valueOf(200), 50, Instant.now(), Instant.now());
    when(ticketTypeService.create(eq(eventId), any())).thenReturn(response);

    mockMvc
        .perform(
            post("/api/v1/events/{eventId}/ticket-types", eventId)
  ...
  void shouldListTicketTypesForEvent() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/events/{eventId}/ticket-types", eventId))
        .andExpect(status().isOk())
  ...
  void shouldUpdateTicketType() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/events/{eventId}/ticket-types/{id}", eventId, ticketTypeId)
                .header("X-User-Id", "user-123")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk())
  ...
  void shouldDeleteTicketType() throws Exception {
    mockMvc
        .perform(
            delete("/api/v1/events/{eventId}/ticket-types/{id}", eventId, ticketTypeId)
                .header("X-User-Id", "user-123"))
