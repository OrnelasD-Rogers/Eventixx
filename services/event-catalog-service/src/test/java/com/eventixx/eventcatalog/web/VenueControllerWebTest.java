...
class VenueControllerWebTest {
  ...
  void shouldCreateVenue() throws Exception {
    ...
    VenueResponse response =
        new VenueResponse(
            venueId, "Arena", "123 St", "SP", "BR", 5000, Instant.now(), Instant.now());
  ...
  void shouldGetVenueById() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/venues/{id}", venueId))
  ...
  void shouldReturn404_whenVenueNotFound() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/venues/{id}", venueId)).andExpect(status().isNotFound());
  }

  @Test
  void shouldListVenues() throws Exception {
    Page<VenueSummaryResponse> page =
        new PageImpl<>(List.of(new VenueSummaryResponse(venueId, "Arena", "SP", "BR")));
  ...
  void shouldUpdateVenue() throws Exception {
    ...
    VenueResponse response =
        new VenueResponse(
            venueId, "Updated Arena", "123 St", "SP", "BR", 5000, Instant.now(), Instant.now());
  ...
  void shouldDeleteVenue() throws Exception {
    mockMvc
        .perform(delete("/api/v1/venues/{id}", venueId).header("X-User-Id", "user-123"))
