...
class VenueCatalogIntegrationTest extends CatalogIntegrationTestBase {
  ...
  void shouldUpdateVenue() {
    var venue = createVenue();
    var update =
        new UpdateVenueRequest("Updated Arena", "456 New St", "New City", "New Country", 200);
  ...
  void shouldReturn404AfterSoftDelete() {
    ...
    restClient.delete().uri("/api/v1/venues/{id}", venue.id()).headers(requestHeaders()).exchange();

    restClient.get().uri("/api/v1/venues/{id}", venue.id()).exchange().expectStatus().isNotFound();
  }
