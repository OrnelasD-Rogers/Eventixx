...
class EventCatalogIntegrationTest extends CatalogIntegrationTestBase {
  ...
  void shouldReturn404AfterSoftDelete() {
    ...
    restClient.delete().uri("/api/v1/events/{id}", event.id()).headers(requestHeaders()).exchange();

    restClient.get().uri("/api/v1/events/{id}", event.id()).exchange().expectStatus().isNotFound();
  }
  ...
  void shouldReturn400WhenTicketQuantityExceedsCapacity() {
    var venue = createVenue();
    var category = createCategory(1);
    var overCapacityTickets =
        List.of(new CreateTicketTypeRequest("Standard", BigDecimal.valueOf(50), 150));
